 ...
 * - Q4 无关三路径：(不在书架 + 无深度) OR (在书架 + 从未打开 + 无深度) OR (在书架 + 读完 + 无深度)
 * - Q2 承诺债务收紧：在书架 + 有阅读痕迹 + 未读完 + 无深度
 * - 推荐种子：Q1 ∪ Q3（禁止用 Q2）
 * - Q2 不主动弹窗纪律
 *
 * @author fxbin
...
function classifyQuadrant(input: BookSignalInputs, isDeep: boolean): QuadrantKind {
  ...
  if (input.onShelf && !isDeep && !input.finishReading && input.hasReadActivity) return 'commitment_debt';
